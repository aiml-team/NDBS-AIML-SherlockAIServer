...
async def process_json_data(request_data: dict):
    ...
    try:
        ...
        # Pull prospect_name out of the data payload (it was injected by the
        # Web App into master_data.json) so it doesn't get treated as a section.
        # Also accept it as a top-level field on the request for flexibility.
        prospect_name = (
            request_data.get("prospect_name")
            or (summarized_data.pop("prospect_name", None) if isinstance(summarized_data, dict) else None)
            or ""
        )
        print(f"Prospect name for cover page: {prospect_name!r}")

        # Pull the internet-research flag the same way. The Web App stamps
        # `_internet_research_used = True` on master_data when Tavily filled
        # at least one Section 1 field. The leading underscore keeps it from
        # being mistaken for a section by the renderer's iterators, but we
        # pop it defensively here as well. Also accept it at the request top
        # level so the flag can be set without mutating master_data.
        ...
        print(f"Internet research used (Section 1 disclaimer): {internet_research_used}")
        ...
        document_bytes = generate_document_in_memory(
            config.TEMPLATE_FILE, summarized_data,
